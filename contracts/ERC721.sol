    ...
    IERC721Errors
{
    ...
    function _safeMint(
        ...
        _mint(to, _tokenURI);
        if (!_checkOnERC721Received(address(0), to, _totalSupply, data)) {
            revert ERC721InvalidReceiver(to);
        }
    }
     ...
     * Emits a {Transfer} event.
     */
    function _mint(address to, string memory _tokenURI) internal virtual {
        if (to == address(0)) {
            revert ERC721InvalidReceiver(address(0));
        }
        if (_exists(_totalSupply + 1)) {
            revert ERC721InvalidSender(address(0));
        }

        _beforeTokenTransfer(address(0), to, _totalSupply + 1, 1);
        ...
        _tokenURIs[_totalSupply] = _tokenURI;

        emit Transfer(address(0), to, _totalSupply);

        _afterTokenTransfer(address(0), to, _totalSupply, 1);
    }
